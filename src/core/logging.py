...
This module provides a centralized logging configuration that includes:
- Console logging with rich formatting
- File logging with JSON format and rotation
- Phone number masking functionality
...
log_file_path = os.getenv(
    "DJANGO_LOG_FILE", default="/vol/web/logs/logfile.log"
)
...
class PhoneNumberFilter(logging.Filter):
    ...
    def __init__(self, name: str = "", display_digits: int = 4) -> None:
        """
        Initialize the PhoneNumberFilter.

        Creates a new filter instance with the specified name and number
        of digits to display from phone numbers.
    ...
    def filter(self, record: 'logging.LogRecord') -> bool:
        ...
        Args:
            record (logging.LogRecord): The log record potentially
...
def logger_config() -> None:
    ...
    and configures environment-specific log levels.
    """
    dictConfig(
        {
            "version": 1,
            ...
            "filters": {
                "hide_phone": {
                    "()": PhoneNumberFilter,
                    "display_digits": (
                        10 if environment == "development" else 4
                    ),
                },
            },
            "formatters": {
                "standard": {
                    ...
                    "format": (
                        "%(asctime)-10s| %(name)s(line: %(lineno)d)| "
                        "%(message)s"
                    ),
                },
                "file": {
